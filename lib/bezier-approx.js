...
function minDistanceToQuad(point, p1, c1, p2) {
  ...
  // The distance between given point and quadratic curve is equals to
  // sqrt((f(t) - point)^2), so these expressio has zero derivative by t at points where
  // (f'(t), (f(t) - point)) = 0.
  ...
  // So to find the minimum distance one have to just pick the minimum value of
  // the distance one set {t = 0 | t = 1 | t is root of the equation from [0, 1] }.
...
function cubicToQuad(p1x, p1y, c1x, c1y, c2x, c2y, p2x, p2y, errorBound) {
  ...
  if (a < 0.1) { // the curve is actually quadratic, which means that it came from TTF font
    // so we could just convert it to quadratic using equation
    // 0*t^3 + b*t^2 + c*t + d = (p1 + p2 - 2*c1)*t^2 + 2*(c1 - p1) + p1
    return toFlatArray([ p1, b.sub(p1.add(p2)).div(2),  p2 ]);
  }
 ...
 *
 */
function isInLine(p1x, p1y, mx, my, p2x, p2y, accuracy) {
  var p1 = new Point(p1x, p1y);
